...
POKEMON_ATTRIBUTES = {"Ability": "ability", "Color": "pokemon-color", "Type": "type"}
...
def get_pokemon_by_name(name):
    ...
    try:
        ...
        pokemon = {
            ...
            "abilities": [
                item["ability"]["name"] for item in response_json["abilities"]
            ],
...
def get_pokemon_description(pokemon):
    ...
    description = next(
        filter(
            lambda x: x["language"]["name"] == "en",
            response_json["flavor_text_entries"],
        ),
        None,
    )["flavor_text"].replace("\n", " ")
